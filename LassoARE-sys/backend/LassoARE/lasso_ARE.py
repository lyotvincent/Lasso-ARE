...
class LassoARE:
    ...
    def train_adversarial(self, X, X_raw, size_factor, user_selected_lists=None,
        ...
        with torch.no_grad():
            ...
            if num_groups > 0 and num_groups < self.n_clusters:
                ...
                if other_features.shape[0] >= num_other_clusters > 0:
                    print(f"Running KMeans initialization ({num_other_clusters} clusters, n_init=20)...", flush=True)
                    ...
                    print(f"Running KMeans initialization ({self.n_clusters} clusters, n_init=20)...", flush=True)
        ...
        for epoch in range(n_epochs):
            ...
            # Print detailed progress periodically while still emitting the epoch header every round.
