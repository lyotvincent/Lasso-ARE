...
from .utils import DispAct, MeanAct, check_matrix
...
def reconstruction_with_lasso_are(adata, user_selected_lists, using_emb=None, n_clusters=None, 
    ...
    print("Inferring LassoARE cluster count...", flush=True)
    if n_clusters is None:
        if 'codes' in adata.obs.columns:
            ...
            print("Computing neighbor graph for cluster inference...", flush=True)
            sc.pp.neighbors(adata)
            print("Running Leiden for cluster inference...", flush=True)
            sc.tl.leiden(adata)
            annotations = adata.obs['leiden']
    ...
    if is_pca:
        ...
        existing_pca = process_adata.obsm['X_pca'] if 'X_pca' in process_adata.obsm else None
        if existing_pca is not None and existing_pca.shape[1] >= pca_dimension:
            print(f"Reusing precomputed X_pca ({existing_pca.shape[1]} dimensions) for LassoARE.", flush=True)
            process_adata.obsm['X_pca'] = np.asarray(existing_pca[:, :pca_dimension], dtype=np.float32)
            raw_pca_key = 'X_pca_raw_LassoARE'
            if raw_pca_key in adata.obsm and adata.obsm[raw_pca_key].shape[1] >= pca_dimension:
                print(f"Reusing precomputed {raw_pca_key} for LassoARE raw target.", flush=True)
                raw_X_pca = np.asarray(adata.obsm[raw_pca_key][:, :pca_dimension], dtype=np.float32)
            else:
                print("Precomputed raw PCA not found; using X_pca as raw target.", flush=True)
                raw_X_pca = process_adata.obsm['X_pca'].copy()
        else:
            print(f"Running PCA on input matrix ({original_dim} -> {pca_dimension})...", flush=True)
            print(f"Applying PCA: reducing from {original_dim} to {pca_dimension} dimensions...", flush=True)
            ...
            if using_emb is None and has_raw:
                print("Running PCA on raw counts...", flush=True)
    ...
    print("Preparing LassoARE matrices...", flush=True)
    ...
    print("Initializing LassoARE model...", flush=True)
    ...
    print("Storing LassoARE outputs in AnnData...", flush=True)
...
def reconstruction_with_ref(adata, user_selected_lists, using_emb, n_clusters=None,
    ...
    if is_pca:
        ...
        rsc_pca = adata.uns.get('LassoARE_rsc_pca', {}) if isinstance(adata.uns.get('LassoARE_rsc_pca', {}), dict) else {}
        pca_key = rsc_pca.get('embedding_pca_key') if rsc_pca.get('embedding_key') == using_emb else None
        if pca_key and pca_key in adata.obsm and adata.obsm[pca_key].shape[1] >= pca_dim:
            print(f"Reusing precomputed {pca_key} ({adata.obsm[pca_key].shape[1]} dimensions) for embedding PCA.", flush=True)
            emb_data = np.asarray(adata.obsm[pca_key][:, :pca_dim], dtype=np.float32)
        else:
            print(f"Running PCA on embedding ({emb_data.shape[1]} -> {pca_dim})...", flush=True)
            print(f"Applying PCA to embedding: {emb_data.shape[1]} → {pca_dim}", flush=True)
    ...
    # ------------------------------------------------------------------
    print("Preparing expression reference matrices...", flush=True)
    has_raw = hasattr(adata, 'raw') and adata.raw is not None
    ...
    print("Initializing reference-guided LassoARE model...", flush=True)
    ...
    with torch.no_grad():
        print("Encoding data for KMeans initialization...", flush=True)
        ...
        if num_groups > 0 and num_groups < n_clusters:
            ...
            if other_feat.shape[0] >= n_other > 0:
                print(f"Running KMeans initialization ({n_other} clusters, n_init=20)...", flush=True)
            ...
            print(f"Running KMeans initialization ({n_clusters} clusters, n_init=20)...", flush=True)
    ...
    if num_groups > 0 and disc_pretrain_epoch > 0:
        ...
        for ep in range(disc_pretrain_epoch):
            ...
            for indices, xb, xrb, sfb, _zref_b in dataloader:  # _zref_b unused here
                ...
                epoch_batches += 1
            avg_dloss = epoch_dloss / max(epoch_batches, 1)
            print(f"  Disc pretrain [{ep+1}/{disc_pretrain_epoch}], loss: {avg_dloss:.4f}")
    ...
    # 8. Collect results
    # ------------------------------------------------------------------
    print("Collecting final LassoARE latent representation...", flush=True)
    with torch.no_grad():
        main_lasso.generator.eval()
