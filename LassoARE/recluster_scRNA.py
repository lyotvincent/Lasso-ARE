...
def fast_neighbors_faiss(adata, use_rep, n_neighbors=100):
    ...
    import faiss
    """Accelerate neighbors calculation using FAISS"""
    ...
    # Build connectivities and distances sparse matrices
    rows = np.repeat(np.arange(n_cells), n_neighbors)
    cols = indices.flatten()
    dist_vals = distances.flatten()
    
    # Convert to connectivity weights
    sigma = np.mean(distances[:, 1])  # Average distance to the first non-self neighbor
...
def recluster_with_lasso_are(adata, user_selected_lists, n_clusters=None, 
                             ...
                             is_pca=False, pca_dimension=500, do_pp=False, do_harmony=True):
    """
    ...
    Args:
        ...
        do_harmony: whether to apply Harmony integration
    ...
    if is_pca:
        ...
        if 'batch' in adata.obs.columns:
            if do_harmony:
                print("Applying Harmony integration on PCA results...")
        ...
        if hasattr(adata, 'raw') and adata.raw is not None:
            ...
            if 'batch' in raw_adata.obs.columns:
                if do_harmony:
                    print("Applying Harmony integration on raw PCA results...")
...
def recluster_with_gan(adata, user_selected_list, n_clusters=None, 
                      ...
                      lambda_attention=1.0, lambda_feature=1.0, lambda_consistency=1.0,
                      leiden_r=0.2,
                      is_pca=False, pca_dimension=500, use_rsc=False, do_pp=False, do_harmony=True):
    """
...
def recluster_with_existing_pca(adata, user_selected_lists, n_clusters=None, 
    ...
    if 'X_pca' not in adata.obsm:
        raise ValueError("adata.obsm['X_pca'] does not exist, please run PCA externally before calling this function.")
    
    # If Harmony needs to be executed within this function
    if do_harmony:
        if 'batch' not in adata.obs.columns:
            print("Warning: batch column not found, skipping Harmony.")
        else:
            pp.harmony_integrate(adata, key='batch')
            adata.obsm['X_pca'] = adata.obsm['X_pca_harmony']
            existing_harmony = True
    
    # Select PCA representation to use
    if existing_harmony and 'X_pca_harmony' in adata.obsm:
        x = adata.obsm['X_pca_harmony']
    else:
        x = adata.obsm['X_pca']
    
    # Raw counts (fallback to PCA matrix if dimensionality does not match PCA, to avoid ZINB dimension mismatch)
    if hasattr(adata, 'raw') and adata.raw is not None:
        raw_X = adata.raw.X.todense() if scipy.sparse.issparse(adata.raw.X) else adata.raw.X
        raw_X = np.array(raw_X)
    else:
    ...
    # Auto-estimate the number of clusters
    if n_clusters is None:
    ...
    # Normalize user selection format
    if user_selected_lists is None:
        user_selected_lists = []
    elif not isinstance(user_selected_lists, list):
        user_selected_lists = [user_selected_lists]
    ...
    # Optional downstream graph embedding and clustering
    if do_pp:
        leiden_adata = adata.copy()
        sc.pp.neighbors(leiden_adata, use_rep='LassoARE_latent', n_neighbors=100)
        sc.tl.umap(leiden_adata, min_dist=0.5, spread=0.8)
